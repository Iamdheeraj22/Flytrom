...
import com.flytrom.learning.databinding.ActivityChaptersBinding;
...
public class ChaptersActivity extends BaseActivity<ActivityChaptersBinding>
    ...
    protected void onViewReady(Bundle savedInstanceState, Intent intent) {
        ...
        Glide.with(this).load(Constants.MEDIA_URL + image).into(binding.lectureImgQbank);
    ...
    private void initView() {
        ...
        mChaptersAdapter = new SimpleRecyclerViewAdapter<>(R.layout.q_bank_play_list,
                BR.bean, new SimpleRecyclerViewAdapter.SimpleCallback<ChapterBean>() {
            @Override
        ...
        binding.recyclerVideos.setAdapter(mChaptersAdapter);
...
//
//        if (mSubjectBean != null) {
//            mCallChapters = AppController.getInstance().getApis().getChapters(getHeader(), String.valueOf(currentPage), mSubjectBean.getTitle(), Constants.PAID);
//        } else {
//            mCallChapters = AppController.getInstance().getApis().getChapters(getHeader(), String.valueOf(currentPage), null, Constants.FREE);
//        }
//
...
//                                float totalQBank = Float.parseFloat(bankChapterBean.getData().get(i).getTotal_mcqs());
...
//    }
    ...
    private void getAllChapters(int currentPage, boolean progress) {
        ...
        if (mSubjectBean != null) {
            ...
            mCallChapters = AppController.getInstance().getApis().getChapters(getHeader(), String.valueOf(currentPage), null, Constants.PAID);
        }
                        ...
                        if (bankChapterBean.getData() != null && bankChapterBean.getData().size() > 0) {
                            ...
                            for (int i = 0; i < bankChapterBean.getData().size(); i++) {
                                if (bankChapterBean.getData().get(i).getTotal_mcqs().equalsIgnoreCase("0")){
                                    bankChapterBean.getData().get(i).setMonth_name(String.valueOf(0));
                                }else {
                                    if (bankChapterBean.getData().get(i).getSavedData().size() != 0){
                                      ...
                                      bankChapterBean.getData().get(i).setMonth_name(String.valueOf(total));
                                    }else {
                                        bankChapterBean.getData().get(i).setMonth_name(String.valueOf(0));
                            ...
                            if (bankChapterBean.getData().size() == 1 || bankChapterBean.getData().size() == 0){
                                binding.subjectLectureQbank.setText(bankChapterBean.getData().size()+" Chapter");
    ...
    private BaseCallback baseCallback = view -> {
        switch (view.getId()) {
            case R.id.card_back_lectureQbank:
                goBack();
                break;
    ...
    private void showEmptyView() {
        ...
        binding.linearEmptyViewQbank.setVisibility(View.VISIBLE);
    }
